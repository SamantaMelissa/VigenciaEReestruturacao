-- Libera temporariamente todas as ações da Central de Validações para qualquer
-- usuário autenticado, inclusive registrar o retorno e excluir pendências.
...
drop policy if exists "Admins delete validations" on public.school_validations;
drop policy if exists "Authenticated users delete validations" on public.school_validations;
...
begin
  ...
  if target_evaluation.id is null then
    insert into public.evaluations(course_code,course_name,criterion_key,criterion_label,status,current_question,
    ...
    values(validation.course_code,validation.course_name,validation.criterion_key,validation.criterion_label,calculated_status,
      ...
      jsonb_build_object('answers',coalesce(p_trail,'[]'::jsonb),'decisionPath',coalesce(p_trail,'[]'::jsonb),
        'enrollments',validation.enrollments,'units',validation.units,'awaitingSchoolValidation',false,
    ...
    update public.evaluations set status=calculated_status,
      ...
      state=coalesce(state,'{}'::jsonb)||jsonb_build_object('answers',coalesce(p_trail,'[]'::jsonb),
        'decisionPath',coalesce(p_trail,'[]'::jsonb),'enrollments',validation.enrollments,'units',validation.units,
