...
-- Aplica o retorno da Central de Validações sem duplicar a avaliação.
...
begin
  ...
  calculated_result := case when p_positive then null else 'FECHAR A VIGÊNCIA' end;
  calculated_justification := case when p_positive then null else
  ...
  if target_evaluation.id is null then
    insert into public.evaluations (
      ...
      validation.course_code, validation.course_name, validation.criterion_key, validation.criterion_label,
      calculated_status, case when p_positive then calculated_next else null end,
      calculated_result, calculated_justification,
      jsonb_build_object(
