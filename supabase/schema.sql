...
begin
  ...
  insert into public.audit_events(entity_type,entity_id,action,old_data,new_data,actor_id)
  values('evaluation',target.id::text,'assign_mapped_area',to_jsonb(target),jsonb_build_object('mapped_area',p_mapped_area),auth.uid());
  update public.evaluations evaluation set state=coalesce(evaluation.state,'{}'::jsonb)||jsonb_build_object(
    'mappedAreasByTitle',jsonb_build_array(p_mapped_area),'mappedAreasLabel',p_mapped_area,
