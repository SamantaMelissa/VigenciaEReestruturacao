...
-- Torna o retorno da Central de Validações idempotente por curso e evita que
-- uma pendência sem evaluation_id crie outra conclusão para o mesmo código.
...
begin
  ...
  from public.school_validations
  where id = p_validation_id
  for update;

  if not found then
    raise exception 'Validação não encontrada.';
  end if;

  -- Serializa qualquer retorno referente ao mesmo curso, inclusive quando
  -- existem duas pendências antigas abertas simultaneamente.
  perform pg_advisory_xact_lock(hashtextextended(validation.course_code, 0));

  select * into target_evaluation
  from public.evaluations
  where course_code = validation.course_code
  ...
  if target_evaluation.id is null and validation.evaluation_id is not null then
    select * into target_evaluation
    from public.evaluations
    where id = validation.evaluation_id
    for update;
  end if;

  -- Esta busca faltava no fluxo anterior: reutiliza a avaliação já concluída
  -- quando uma validação antiga estiver sem vínculo.
  ...
  calculated_status := case
    when p_positive then 'rascunho'::public.evaluation_status
    else 'concluida'::public.evaluation_status
  end;
  ...
  if target_evaluation.id is null then
    insert into public.evaluations (
      ...
      validation.course_code, validation.course_name, validation.criterion_key,
      validation.criterion_label, calculated_status,
      case when p_positive then calculated_next else null end,
      case when p_positive then null else 'FECHAR A VIGÊNCIA' end,
      case when p_positive then null else
        'A unidade não apresentou justificativa técnica para manter o curso. Diante do retorno registrado na Central de Validações, recomenda-se fechar a vigência.' end,
      jsonb_build_object(
    ...
    set status = calculated_status,
        ...
        justification = case when p_positive then null else
          'A unidade não apresentou justificativa técnica para manter o curso. Diante do retorno registrado na Central de Validações, recomenda-se fechar a vigência.' end,
  ...
  update public.school_validations
  set evaluation_id = target_evaluation.id
...
-- Conferência somente leitura: deve retornar zero linhas após novos retornos.
select course_code, max(course_name) as course_name, count(*) as quantity
from public.evaluations
where status = 'concluida'
group by course_code
having count(*) > 1
order by quantity desc, course_code;
